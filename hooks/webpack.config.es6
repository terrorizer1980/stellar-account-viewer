// File replacement for "node_modules/interstellar/lib/webpack.config.es6"
// Changed lines commented with `@Trezor`
// - webpack fails on "fs" import
// - webpack doesn't resolve "browser" field of "trezor-connect" package properly
...
var baseConfig = {
  ...
  plugins: [
    ...
    new webpack.IgnorePlugin(/^fs$/), // @Trezor: ignore "fs" module (./bindings)
    // @Trezor: connect "browser" field is ignored by webpack :start
    ...
    new webpack.NormalModuleReplacementPlugin(/env\/node\/networkUtils$/, '../env/browser/networkUtils'),
     // @Trezor: connect "browser" field :end
...
var environments = {
  ...
  prd: {
    mode: "production", // @Trezor: add build mode
    devtool: "source-map",
    plugins: [
